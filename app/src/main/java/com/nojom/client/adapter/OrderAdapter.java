...
import com.nojom.client.databinding.ItemOrdersTextBinding;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int listPosition) {
        ...
        if (item.isHeaderType) {
            ...
            if (item.profiles != null && item.profiles.size() > 0) {
                ...
                addOverlappingImages(((SimpleViewHolder) holder).binding.imageContainer, item.profiles);
            } else {
                ((SimpleViewHolder) holder).binding.imageContainer.removeAllViews();
            ...
            if (date1 != null) {
                if (activity.printDifference(date1, date).equalsIgnoreCase("0")) {
                    ...
                    ((SimpleViewHolder) holder).binding.txtDate.setText(activity.getString(R.string.due_date) + result);
                } else {
                    String finalDate = dfFinal2.format(date1);
                    ((SimpleViewHolder) holder).binding.txtDate.setText(activity.getString(R.string.due_date) + finalDate);
                }
            }
