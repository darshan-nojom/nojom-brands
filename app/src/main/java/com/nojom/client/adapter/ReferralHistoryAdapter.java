...
import com.nojom.client.databinding.ItemReferralHistoryBinding;
    ...
    public void onBindViewHolder(@NotNull final SimpleViewHolder holder, final int position) {
        ...
        if (position == 0) {
            ...
            if (activity.getLanguage().equals("ar")) {
                holder.binding.txtName.setText(String.format(Locale.US, "%s-%d", item.username, position));
            } else {
                holder.binding.txtName.setText(String.format(Locale.US, "%d-%s", position, item.username));
            }
            ...
            SimpleDateFormat spf = new SimpleDateFormat("yyyy-MM-dd'T'hh:mm:ss", Locale.getDefault());
            Date newDate = null;
            try {
                newDate = spf.parse(item.timestamp);
                if (activity.getLanguage().equals("ar")) {
                    spf = new SimpleDateFormat("dd-MM-yyyy hh:mm a", Locale.getDefault());
                } else {
                    spf = new SimpleDateFormat("yyyy-MM-dd hh:mm a", Locale.getDefault());
                }
