...
import com.nojom.client.databinding.ItemInfServicesBinding;
...
public class InfluencerServiceAdapter extends RecyclerView.Adapter<InfluencerServiceAdapter.SimpleViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final SimpleViewHolder holder, final int position) {
        if (arrUserList != null) {
            ...
            holder.binding.tvReceiverName.setText(item.getPlatformName(baseActivity.getLanguage()));
            holder.binding.tvAmount.setText(baseActivity.getString(R.string.from) + " " + Utils.numberFormat(String.valueOf(item.minPrice)));

            if (!TextUtils.isEmpty(item.platform_icon)) {
                Glide.with(baseActivity).load(item.platform_icon)
