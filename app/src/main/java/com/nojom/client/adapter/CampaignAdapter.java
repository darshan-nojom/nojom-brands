...
public class CampaignAdapter extends RecyclerSwipeAdapter<CampaignAdapter.SimpleViewHolder> implements RequestResponseListener, BaseActivity.ExpertInfoListener {
    ...
    public CampaignAdapter(ProjectsListFragment context, OnClickJobListener onClickJobListener) {
    ...
    public SimpleViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ItemProjectsListBinding itemProjectsListBinding = DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()), R.layout.item_projects_list, parent, false);
    ...
    public void onBindViewHolder(final SimpleViewHolder holder, final int position) {
        ...
        if (!TextUtils.isEmpty(item.campaignTitle)) {
            ...
            holder.binding.tvTitle.setText(item.jp_title);
        }

        if (item.campaignId != null) {
            holder.binding.tvJobId.setText(String.format(Locale.US, activity.getString(R.string.jobid) + " : %s", item.campaignId));
        } else if (item.jp_id != null) {
            holder.binding.tvJobId.setText(String.format(Locale.US, activity.getString(R.string.jobid) + " : %s", item.jp_id));
        }
...
//        if (item.job.contains("gig")) {
        ...
        if (item.totalPrice != 0 && item.totalPrice > 0) {
            holder.binding.tvBudget.setText(activity.getCurrency().equals("SAR") ? Utils.decimalFormat(String.valueOf(item.totalPrice)) + " " + activity.getString(R.string.sar) : activity.getString(R.string.dollar) + Utils.decimalFormat(String.valueOf(item.totalPrice)) + "");
        } else {

            holder.binding.tvBids.setVisibility(View.VISIBLE);
            holder.binding.tvDotBids.setVisibility(View.VISIBLE);
            holder.binding.tvBids.setText(item.bids_count > 1 ? item.bids_count + " " + activity.getString(R.string.bids) : item.bids_count + " " + activity.getString(R.string.bid));
            holder.binding.swipe.setSwipeEnabled(true);

            if (item.client_rate_id == 0 && item.budget != null && item.budget != 0) {
                holder.binding.tvBudget.setText(activity.getCurrency().equals("SAR") ? Utils.decimalFormat(String.valueOf(item.budget)) + " " + activity.getString(R.string.sar) : activity.getString(R.string.dollar) + Utils.decimalFormat(String.valueOf(item.budget)) + "");
            } else {
                if (item.cr_id != null && item.cr_id != -1) {
                    if (item.range_to != null && item.range_from != null) {
                        if (activity.getCurrency().equals("SAR")) {
                            holder.binding.tvBudget.setText(String.format(Locale.US, activity.getString(R.string.s_sar_s_sar), Utils.decimalFormat(String.valueOf(item.range_from)), Utils.decimalFormat(String.valueOf(item.range_to))));
                        } else {
                            holder.binding.tvBudget.setText(String.format(Locale.US, "$%s - $%s", Utils.decimalFormat(String.valueOf(item.range_from)), Utils.decimalFormat(String.valueOf(item.range_to))));
                        }
                    } else if (item.range_from != null) {
                        if (activity.getCurrency().equals("SAR")) {
                            holder.binding.tvBudget.setText(String.format(Locale.US, activity.getString(R.string.s_sar), Utils.decimalFormat(String.valueOf(item.range_from))));
                        } else {
                            holder.binding.tvBudget.setText(String.format(Locale.US, "$%s", Utils.decimalFormat(String.valueOf(item.range_from))));
                        }
                    } else if (item.budget != null) {
                        holder.binding.tvBudget.setText(activity.getCurrency().equals("SAR") ? Utils.decimalFormat(String.valueOf(item.budget)) + " " + activity.getString(R.string.sar) : "$" + Utils.decimalFormat(String.valueOf(item.budget)) + "");
                    }
                } else {
    ...
    class SimpleViewHolder extends RecyclerView.ViewHolder {
        ...
        SimpleViewHolder(ItemProjectsListBinding itemView) {
            ...
            binding.llRehire.setOnClickListener(view -> {
                binding.swipe.close();
                try {
                    if (projectsList.get(getAbsoluteAdapterPosition()).clientProfileId > 0) {
                        binding.rlItemview.setBackgroundResource(R.drawable.transp_rounded_corner_10);
                        binding.progressBar.setVisibility(View.VISIBLE);

                        activity.setExpertInfoListener(CampaignAdapter.this);//attach listener
                        adapterPosition = getAbsoluteAdapterPosition();
                        activity.getExpert(projectsList.get(getAbsoluteAdapterPosition()).clientProfileId);
                    } else {
            ...
            binding.llEdit.setOnClickListener(view -> {
                ...
                if (projectsList.get(getAbsoluteAdapterPosition()).campaignId != null) {
                    getProjectById(projectsList.get(getAbsoluteAdapterPosition()).campaignId);
                } else if (projectsList.get(getAbsoluteAdapterPosition()).jp_id != null) {
                    getProjectById(projectsList.get(getAbsoluteAdapterPosition()).jp_id);
                }
            });
            ...
            binding.llDelete.setOnClickListener(view -> {
                binding.swipe.close();
                if (projectsList.get(getAbsoluteAdapterPosition()).campaignId != null) {
                    showDeleteDialog(projectsList.get(getAbsoluteAdapterPosition()).campaignId, binding.swipe, getAbsoluteAdapterPosition());
                } else if (projectsList.get(getAbsoluteAdapterPosition()).jp_id != null) {
                    showDeleteDialog(projectsList.get(getAbsoluteAdapterPosition()).jp_id, binding.swipe, getAbsoluteAdapterPosition());
                }
            ...
            binding.rlItemview.setOnClickListener(view -> {
                ...
                try {
                    if (projectsList.get(getAbsoluteAdapterPosition()).campaignId != null && projectsList.get(getAbsoluteAdapterPosition()).campaignId != UNDER_REVIEW) {
                        if (projectsList.get(getAbsoluteAdapterPosition()).campaignId == Constants.REMOVED) {
                            showRepostDialog(projectsList.get(getAbsoluteAdapterPosition()).campaignId, binding.swipe, getAbsoluteAdapterPosition());
                        } else {
                            binding.rlItemview.setBackgroundResource(R.drawable.transp_rounded_corner_10);
                            binding.progressBar.setVisibility(View.VISIBLE);

                            if (onClickJobListener != null) {
                                onClickJobListener.onClickJob(projectsList.get(getAbsoluteAdapterPosition()).campaignId, getAbsoluteAdapterPosition(), projectsList.get(getAbsoluteAdapterPosition()).type, projectsList.get(getAbsoluteAdapterPosition()).type, projectsList.get(getAbsoluteAdapterPosition()));
                            }
                        }
                    } else {
                        binding.rlItemview.setBackgroundResource(R.drawable.transp_rounded_corner_10);
                        ...
                        if (onClickJobListener != null) {
                            onClickJobListener.onClickJob(projectsList.get(getAbsoluteAdapterPosition()).jp_id, getAbsoluteAdapterPosition(), projectsList.get(getAbsoluteAdapterPosition()).job, projectsList.get(getAbsoluteAdapterPosition()).gigType, projectsList.get(getAbsoluteAdapterPosition()));
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
