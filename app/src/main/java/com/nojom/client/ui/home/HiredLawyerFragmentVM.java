...
class HiredLawyerFragmentVM extends AndroidViewModel implements LawyerPopularAdapter.OnClickListener, SaveRemoveClickListener, RequestResponseListener {
    ...
    HiredLawyerFragmentVM(Application application, FragmentAllPopularLawyerBinding chatListBinding, BaseFragment chatListFragment) {
        super(application);
    ...
    public void initData(boolean isCallApi) {
        binding.noData.tvNoTitle.setText(fragment.getString(R.string.no_hired_expert));
        binding.noData.tvNoDescription.setText(fragment.getString(R.string.all_experts_list_will_be_displayed_here));
        ...
        pageNo = 1;
        if (isCallApi) {
            getExperts(false, ((FindExpertActivity) fragment.activity).getServiceCategoryId(), "HIRED", pageNo);
        }
    ...
    private void setAdapter(List<InfluencerList.AllData> data, String filePath) {
        if (expertList != null && expertList.size() > 0) {
            if (servicesAdapter == null) {
                ...
                servicesAdapter = new LawyerPopularAdapter(fragment.activity, expertList, HiredLawyerFragmentVM.this, filePath, isFromJobScreen);
    ...
    public void getExperts(boolean isClearAll, int serviceCatId) {
        getExperts(isClearAll, serviceCatId, "HIRED", 1);
    }
...
//                Intent i = new Intent(fragment.activity, FreelancerProfileActivity.class);
    ...
    private void notifyProfileProgress() {
        if (servicesAdapter != null && servicesAdapter.getData() != null
                && servicesAdapter.getData().size() > 0 && selectedPos != -1) {
